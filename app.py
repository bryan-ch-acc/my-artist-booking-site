...
from forms import *
from models import db, Venue, Artist, Show, venue
...
def venues():
  ...
  try:
    ...
    for venue in venue_list:
      ...
      print("Number of upcoming shows:", num_upcoming_shows)
      data = [{
        "city": venue.city,
        "state": venue.state,
        "venues": [{
...
def show_venue(venue_id):
  ...
  try:
    ...
    if not venue_found:
      flash('Artist not found with ID: ' + str(venue_id))
...
def create_venue_submission():
  try:
    # seeking_t = True if request.form.get('seeking_talent') == 'y' else False
    ...
    print("Form data venue obj:", vars(venue))
    ...
    db.session.rollback()
    flash('An error occurred. Venue ' + request.form['name'] + ' could not be listed. Exception: ' + str(e))
    return render_template('pages/home.html')
  finally:
    db.session.close()
...
def artists():
  ...
  try:
    ...
    data = [{"id": artist.id, "name": artist.name} for artist in artists_list]
...
def show_artist(artist_id):
  ...
  try:
    ...
    if not artist_found:
      flash('Artist not found with ID: ' + str(artist_id))
      return redirect(url_for('index'))
...
def edit_artist(artist_id):
  try:
    ...
    form.website_link.data = artist_found.website_link
...
def create_artist_submission():
  try:
    ...
    db.session.rollback()
    flash('An error occurred. Artist ' + request.form['name'] + ' could not be listed. Exception: ' + str(e))
    return render_template('pages/home.html')
  finally:
     db.session.close()
...
def create_show_submission():
  try:
    ...
    if not artist or not venue:
      flash('Artist or Venue not found. Please check the IDs and try again.')
      return render_template('pages/home.html')
    ...
    print("Form data show obj:", vars(show))
    db.session.add(show)
    db.session.commit()
    # on successful db insert, flash success
    flash('Show of ' + artist.name + ' in venue ' + venue.name + ' was successfully listed!')
    return render_template('pages/home.html')
  except Exception as e:
    db.session.rollback()
    flash('An error occurred. Show of ' + artist.name + ' could not be listed. Exception: ' + str(e))
    return render_template('pages/home.html')
  finally:
    db.session.close()
